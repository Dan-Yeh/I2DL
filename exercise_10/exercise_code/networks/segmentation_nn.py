...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            # 3 [64, 64] -> [120, 120]
            nn.UpsamplingBilinear2d(scale_factor=120/64),
            nn.Conv2d(filters[3], filters[4], 1),
            nn.ReLU(),
            # 4 [120] -> [240]
            nn.ConvTranspose2d(filters[4], filters[5], 4, stride=2, padding=1),
            nn.ReLU(),
            # output
            nn.Conv2d(filters[5], 3, 1)
        )
...
class Autoencoder(pl.LightningModule):
    ...
    def configure_optimizers(self):
        LR = 0.1
        optim = torch.optim.Adam(self.decoder.parameters(), LR)
        scheduler = torch.optim.lr_scheduler.StepLR(
            optim, step_size=5, gamma=0.1)
